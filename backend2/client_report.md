...
### 3.3. Услуги текущего мастера
**URL:** `GET /api/services/masters/me`  
**Аутентификация:** Требуется
...
**Примечание:** Возвращает услуги текущего аутентифицированного мастера (используется ID из JWT токена).

---

### 3.4. Свободные кварталы для услуги
...
### 5.1. Список оплат текущего мастера
**URL:** `GET /api/payments/me`  
**Аутентификация:** Требуется
...
[
  {
    "date": "2025-11-30",
    "time": "11:30",
    "service_title": "Коррекция бровей",
  ...
  },
  {
    "date": "2025-11-29",
    "time": "14:30",
    "service_title": "Макияж вечерний",
    "client_full_name": "Петр Петров",
    "amount": "5000.00"
  }
]
```

**Примечание:** Возвращает все оплаты для текущего аутентифицированного мастера через его записи. Время преобразуется из квартала в формат HH:MM (quarter 1 = 8:00, quarter 2 = 8:30, и т.д.).
...
- Запись (appointment) должна существовать
- При ошибке валидации возвращается 400 Bad Request или 404 Not Found
